...
async fn agent_calls_write_tool(world: &mut AgentWorld) {
    ...
    let has_write = tool_use_events
        .iter()
        .any(|e| e.tool_name.as_deref() == Some("write"));
    assert!(
        has_write,
        "Expected Agent to call write tool, but got: {:?}",
        tool_use_events
            .iter()
...
async fn agent_calls_read_tool(world: &mut AgentWorld) {
    let tool_use_events = world.tool_use_events();
    let has_read = tool_use_events
        .iter()
        .any(|e| e.tool_name.as_deref() == Some("read"));
